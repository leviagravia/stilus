...
static void airpad_connect_signals(struct AirpadDataApplication *data_application)
{
    ...
    data_application->data_signals->menu_item_options_append_newline_toggled = g_signal_connect(data_application->data_window->data_menu_bar->menu_item_options_append_newline, "toggled", G_CALLBACK(&airpad_options_update_append_newline), &(data_application->data_options->append_newline));
